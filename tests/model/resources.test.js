...
test("parseTagged survives a truncated payload and a daemon error", () => {
  ...
  eq(Model.parseTagged("Cannot connect to the Docker daemon"), { records: [], unused: [] })
})
...
test("normalizeImage folds a docker images row into the shared shape", () => {
...
test("a container count Docker withholds counts as in use, never as junk", () => {
...
test("a sha256 image id is cut to the twelve characters Docker shows", () => {
...
test("normalizeVolume reads the compose labels and the prune verdict", () => {
  ...
  eq(volume.inUse, true, "docker did not list it as prunable")
  eq(volume.anonymous, false)
  eq(volume.sizeBytes, -1, "the cheap listing does not carry sizes")
})

test("a volume docker would prune is the one marked unused", () => {
...
test("an anonymous volume is recognised by its label or by its 64-hex name", () => {
  ...
  eq(byName.id, hex, "while the full name is what reaches docker volume rm")

  const [byLabel] = Model.normalizeVolumes([volumeRow({ Labels: "com.docker.volume.anonymous=" })], [])
  eq(byLabel.anonymous, true)
})

test("mergeVolumeUsage folds in the sizes only docker system df -v knows", () => {
...
test("a volume name that is not a volume name never reaches docker volume rm", () => {
...
test("the three networks Docker made itself are never offered for removal", () => {
  const nets = Model.normalizeNetworks([
    networkRow({ ID: "aaa1", Name: "bridge", Labels: "" }),
    networkRow({ ID: "aaa2", Name: "host", Labels: "" }),
    networkRow({ ID: "aaa3", Name: "none", Labels: "" }),
    networkRow({ ID: "aaa4", Name: "shop_default" })
  ], ["bridge", "host", "none", "shop_default"])
  eq(nets.map(n => n.builtin), [true, true, true, false])
  eq(nets.map(n => n.inUse), [true, true, true, false],
    "a predefined network reads as in use however docker filters it")
})

test("a predefined network says so instead of pretending to have a project", () => {
  const [row] = rowsOf("networks", Model.normalizeNetworks([networkRow({ Name: "bridge", Labels: "" })], []))
  ok(row.subtitle.indexOf("predefined") !== -1)
})

test("nothing offers a button that can only ever fail", () => {
  const [builtin] = rowsOf("networks", Model.normalizeNetworks([networkRow({ Name: "bridge", Labels: "" })], []))
  eq(builtin.removable, false)
  eq(Model.actionsFor(builtin).map(a => a.verb), ["copy"], "docker will not remove its own networks")
...
test("a click on the row body copies the string you would actually paste", () => {
  ...
  eq(Model.copyValue("volumes", volume), "/var/lib/docker/volumes/shop_pgdata/_data")
...
test("every removal command is built from a validated identifier", () => {
  eq(Model.removeCommand("containers", "ed42e44629ec"), ["docker", "rm", "ed42e44629ec"])
  eq(Model.removeCommand("images", "708e7c2a26ec"), ["docker", "rmi", "708e7c2a26ec"])
  eq(Model.removeCommand("volumes", "shop_pgdata"), ["docker", "volume", "rm", "shop_pgdata"])
  eq(Model.removeCommand("networks", "f638321f8a24"), ["docker", "network", "rm", "f638321f8a24"])
})
...
test("the empty line says why the tab is empty, not just that it is", () => {
  ...
  eq(Model.emptyText("volumes", Object.assign({}, base, { permissionDenied: true })), "No access to the Docker socket")
  eq(Model.emptyText("volumes", Object.assign({}, base, { daemonReachable: false })), "Docker daemon unreachable")
...
test("errorText keeps the one line Docker actually explained itself on", () => {
  eq(Model.errorText("Error response from daemon: volume is in use - [abc]"),
    "volume is in use - [abc]")
...
test("parseJsonArray reads the one shape docker system df -v answers in", () => {
  eq(Model.parseJsonArray('[{"Name":"a","Size":"1GB"}]'), [{ Name: "a", Size: "1GB" }])
  eq(Model.parseJsonArray("[]"), [])
  eq(Model.parseJsonArray("null"), [])
  eq(Model.parseJsonArray('{"Name":"a"}'), [], "an object is not the array we asked for")
  eq(Model.parseJsonArray('[{"Name":'), [], "and a truncated one is not fatal")
  eq(Model.parseJsonArray(""), [])
  eq(Model.parseJsonArray(null), [])
})
...
test("every action object has exactly the same keys as every other", () => {
  const rows = []
    ...
    .concat(rowsOf("networks", Model.normalizeNetworks([
      networkRow(), networkRow({ ID: "aaa", Name: "bridge", Labels: "" })
    ], [])))
